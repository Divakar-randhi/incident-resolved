...
public class ReportController {
    ...
    @Operation(summary = "Upload Excel file for incident data", description = "Upload an Excel file (.xlsx) to parse incidents, save to DB, and generate report sheet.")
    ...
    public ResponseEntity<Map<String, Object>> uploadExcel(
        ...
        try {
            ...
            if (!savedFile.exists() || savedFile.length() == 0) {
                response.put("error", "Failed to save uploaded file: " + filePath + " (size: " + (savedFile.exists() ? savedFile.length() : "not found") + ").");
  ...
  @Operation(summary = "Generate report from DB", description = "Generates a report from existing DB data and downloads Excel.")
...
public ResponseEntity<?> generateReportFromDB(
    ...
    try {
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(errorResponse);  // FIX: Return JSON error
    }
}
}